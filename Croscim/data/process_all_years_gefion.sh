...
AVAIL_GB=$(df --output=avail -BG "$BASE_OUTPUT" | tail -1 | tr -d ' G')
echo "Espace disponible: ${AVAIL_GB} GB (~134 GB par année)"
echo ""

for YEAR in "${YEARS_LIST[@]}"; do
    SQSH="${YEARS[$YEAR]}"
    EXTRACT_DIR="${BASE_OUTPUT}/squash_${YEAR}_extract"
    OUTPUT_DIR="${BASE_OUTPUT}/data_${YEAR}"
