...
def plot_temporal_statistics(filename, score=False):
    
    ds1 = xr.open_dataset(filename, group='diff')
    ds2 = xr.open_dataset(filename, group='alongtrack')
    nobs = ds1['count'].dropna(dim='time')
    
    if score:
        rmse_score = 1. - ds1['rms']/ds2['rms']
        rmse_score = rmse_score.dropna(dim='time').where(ds1['count'] > 10, drop=True)
        figure = rmse_score.hvplot.line(ylabel='RMSE SCORE', shared_axes=True, color='r') + nobs.hvplot.step(ylabel='#Obs.', shared_axes=True, color='grey')
    else:
        rmse_score = ds1['rms']
        rmse_score = rmse_score.dropna(dim='time').where(ds1['count'] > 10, drop=True)
        figure = rmse_score.hvplot.line(ylabel='RMSE', shared_axes=True, color='r') + nobs.dropna(dim='time').hvplot.step(ylabel='#Obs.', shared_axes=True, color='grey') 
...
def intercomparison_temporal_statistics(list_of_filename, list_of_label, score=False):
