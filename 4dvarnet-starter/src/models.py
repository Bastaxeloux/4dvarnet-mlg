...
class Lit4dVarNet(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        if batch_idx == 0:
            self.test_data = []
            print("=== DÉBUT TEST ===")
        print(f"Batch {batch_idx}: shape du batch = {batch.input.shape}")
        out = self(batch=batch)
        m, s = self.norm_stats
    ...
    def on_test_epoch_end(self):
        ...
        if self.logger:
            # Gérer le cas où log_dir est None (WandbLogger)
            log_dir = getattr(self.logger, 'log_dir', None)
            if log_dir is not None:
                self.test_data.to_netcdf(Path(log_dir) / 'test_data.nc')
                print(Path(log_dir) / 'test_data.nc')
            else:
                # Utiliser un répertoire de sauvegarde par défaut dans /dmidata
                import os
                save_dir = "/dmidata/users/malegu/4dvarnet-outputs/test_data"
                os.makedirs(save_dir, exist_ok=True)
                self.test_data.to_netcdf(Path(save_dir) / 'test_data.nc')
                print(f"Sauvegarde test_data.nc dans {save_dir}")
